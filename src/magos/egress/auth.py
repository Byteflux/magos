...
def maybe_inject_api_key(headers: dict[str, str], target: Target) -> dict[str, str]:
    ...
    if not value:
        raise DispatchError(f"env var {target.api_key_env!r} is not set")
    if target.provider == "anthropic" and value.startswith(_ANTHROPIC_OAUTH_TOKEN_PREFIX):
        return {
            **headers,
            "authorization": f"Bearer {value}",
            "anthropic-beta": _ANTHROPIC_OAUTH_BETA,
        }
    shape = target.auth_header or _default_auth_header(target.provider)
    if shape == "x-api-key":
