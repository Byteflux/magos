...
One endpoint family per sibling module; `payload` holds the shared
LiteLLM payload builder; `sse` holds SSE framing helpers. The
generic `runner` (`proxy_translate` / `stream_translate`) is
the entry point for callers — egress dispatch looks up the per-shape
`ADAPTER` via `TRANSLATE_HANDLERS`. See
`docs/architecture/translation.md`.
"""

from __future__ import annotations

from magos.dispatch.translate.anthropic import ADAPTER as _ANTHROPIC_ADAPTER
from magos.dispatch.translate.openai_chat import ADAPTER as _CHAT_ADAPTER
from magos.dispatch.translate.openai_responses import ADAPTER as _RESPONSES_ADAPTER
