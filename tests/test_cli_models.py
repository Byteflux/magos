...
def test_list_falls_back_to_disk_when_server_unreachable(
    ...
    result = runner.invoke(models_cmd.models_app, ["list"])
...
def test_list_from_disk_skips_server(
    ...
    result = runner.invoke(models_cmd.models_app, ["list", "--from-disk"])
    assert result.exit_code == 0
    assert not called
...
def test_list_format_json_emits_machine_readable_payload(
    ...
    result = runner.invoke(models_cmd.models_app, ["list", "--from-disk", "--format", "json"])
    assert result.exit_code == 0
...
def test_show_returns_nonzero_on_unknown_id(
    ...
    result = runner.invoke(models_cmd.models_app, ["show", "missing/x"])
...
def test_refresh_returns_server_response(
    ...
    result = runner.invoke(models_cmd.models_app, ["refresh", "--provider", "openrouter"])
    assert result.exit_code == 0
...
def test_refresh_returns_nonzero_on_partial_failure(
    ...
    result = runner.invoke(models_cmd.models_app, ["refresh"])
    assert result.exit_code == 1
